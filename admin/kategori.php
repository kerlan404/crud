...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update') {
        ...
        if ($id > 0 && !empty($nama_kategori)) {
            try {
                $pdo->prepare("UPDATE kategori SET nama_kategori = ? WHERE id = ?")->execute([$nama_kategori, $id]);
...
<body class="bg-slate-50 text-slate-800 min-h-screen flex">
                ...
                <table class="w-full text-left text-xs border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php if (empty($categories)): ?>
                            <tr><td colspan="3" class="p-4 text-center text-slate-400">Belum ada data kategori.</td></tr>
                        <?php else: ?>
                            <?php foreach ($categories as $cat): ?>
                                <tr class="hover:bg-slate-50/50 transition">
                                    ...
                                    <td class="p-4 text-right flex justify-end gap-2">
                                        ...
                                        <form id="del-kat-<?= $cat['id'] ?>" action="kategori.php" method="POST" class="inline">
                                            <input type="hidden" name="action" value="delete">
                                            <input type="hidden" name="id" value="<?= $cat['id'] ?>">
                                            <button type="button"
                                                onclick="confirmDeleteKat(<?= $cat['id'] ?>, '<?= htmlspecialchars($cat['nama_kategori'], ENT_QUOTES) ?>')"
    ...
    <script>
        ...
        function confirmDeleteKat(id, name) {
            Swal.fire({
                title: 'Hapus Kategori?',
                html: `Kategori <strong style="color:#CC0000">${name}</strong> akan dihapus permanen.<br><small style="color:#94a3b8">Tidak bisa dihapus jika masih digunakan produk.</small>`,
                icon: 'warning',
                iconColor: '#CC0000',
            ...
            }).then(r => { if (r.isConfirmed) document.getElementById('del-kat-' + id).submit(); });
        }
    </script>
</body>
</html>
